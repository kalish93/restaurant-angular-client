...
export class HasSubscriptionDirective implements OnInit, OnDestroy {

  @Input() appHasSubscription: string[] = [];
  ...
  ngOnInit(): void {
    this.restaurantSubscription = this.restaurantFacade.selectedRestaurant$.subscribe((restaurant) => {
      ...
      const isAllowed: boolean = !!subscriptionLevel && this.appHasSubscription.includes(subscriptionLevel);
